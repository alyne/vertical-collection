...
test('Multiple elements with handlers works as expected', (assert) => {
  ...
  assert.strictEqual(scrollHandlers.length, 2, `We have two elements to watch.`);
  ...
  // test triggering that handler
  assert.strictEqual(scrollable1.scrollTop, 0, `The scrollable is initially unscrolled`);
  assert.strictEqual(scrollable2.scrollTop, 0, `The scrollable is initially unscrolled`);

  afterNextScrollUpdate(() => {
    scrollable1.scrollTop = 10;
    scrollable2.scrollTop = 20;
    assert.strictEqual(scrollable1.scrollTop, 10, `We updated the scrollable's scroll position`);
    assert.strictEqual(scrollable2.scrollTop, 20, `We updated the scrollable's scroll position`);
